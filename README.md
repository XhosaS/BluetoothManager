...
一款面向 Windows 11 x64 的 Flutter 桌面应用，用于在蓝牙耳机的 **A2DP 高音质模式**、**HFP 通话模式**与**自动模式**之间切换。
...
- 自动模式：保持话筒可用，录音开始时由 Windows 进入 HFP，录音结束后恢复 A2DP 高音质传输。
...
- **自动模式状态**：显示“自动 · A2DP 高音质”或“自动 · HFP 通话”。

切换到 HFP 后显示“HFP 未活动”是正常现象；打开录音或通话应用后，Core Audio 会话回调会近实时更新为“HFP 活动中”。应用另保留 30 秒低频看门狗，用于睡眠恢复或驱动漏报时校正状态。
...
1. 打开应用并选择目标蓝牙耳机。
2. 选择 A2DP、HFP 或自动模式；自动模式适合需要在音质和话筒间无人值守切换的场景。
...
- 自动模式不会禁用 HFP 话筒端点；其“A2DP”状态表示话筒空闲时的实际高质量传输。
